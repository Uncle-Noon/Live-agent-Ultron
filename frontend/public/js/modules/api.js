...
export const fetchHistory = (email)                    => fetch(`${API_BASE}/history?email=${encodeURIComponent(email)}`).then(toJson);
export const clearHistory = (email)                    => fetch(`${API_BASE}/history?email=${encodeURIComponent(email)}`, { method: 'DELETE' }).then(toJson);
export const fetchCmds    = (email)                    => fetch(`${API_BASE}/commands?email=${encodeURIComponent(email)}`).then(toJson);
